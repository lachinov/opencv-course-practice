...
int main(int argc, char* argv[])
{
	...
	Ptr<FeatureDetector> detector = ORB::create()/*FeatureDetector::create("ORB")*/;
	Ptr<DescriptorExtractor> descriptorExtractor = ORB::create()/*DescriptorExtractor::create("ORB")*/;
	...
	for (int matchInd = 0; matchInd < matches.size(); matchInd++)
	{
		...
		float scene_dist = cv::norm((Point)point_object.pt - center) * point_object.size / point_scene.size; // scaled
		float scene_angle = angle + (point_scene.angle - point_object.angle) * std::atan(1) / 45.0f;
		
		Point voite = Point(
			(cvRound(point_scene.pt.y + cos(scene_angle)*scene_dist)) / splitSet, 
			(cvRound(point_scene.pt.x + sin(scene_angle)*scene_dist)) / splitSet);

		hgt.at<int>(voite.x, voite.y)++;
		
		object_points.push_back(keypoints1[matches[matchInd].queryIdx].pt);
		scene_points.push_back(keypoints2[matches[matchInd].trainIdx].pt);
	}
	Point maxP;
